...
-- adding the extra layers in the end to be fine-tuned
my_net = nn.Sequential()
my_net:add(SpatialConvolutionMM(4096, total_classes, 1, 1, 1, 1))
my_net:add(nn.View(total_classes))
my_net:add(nn.LogSoftMax())

print '==> here is the overfeat network:'
print(net)

print '==> here is the network to be fine-tuned:'
print(my_net)
---------------------------------------------------------------------------------
...
-- into a 1-dim vector
if net then
  parameters,gradParameters = my_net:getParameters()
end
...
function train()
   ...
   for t = 1,trainData:size(),batchSize do
      ...
      local feval = function(x)
                       ...
                       for i = 1,#inputs do
                          -- estimate f
                          local output = my_net:forward(my_test(inputs[i]))
                          ...
                          my_net:backward(my_test(inputs[i]), df_do)
...
function test()
   ...
   for t = 1,testData:size() do
      ...
      -- test sample
      local pred = my_net:forward(my_test(input))
...
-----------------------------------------------------------------------------------------------------------------------
function my_test(my_input)
  net:evaluate()
  my_output = net:forward(my_input)
  return my_output
end
